...
#include "com/ngos/bootloader_tools/networktest/sections/section1/testcase.h"
...
{ // Ignore CppNgosTraceVerifier
    ...
    UEFI_LI(("NGOS Bootloader Tool - Network Test starting up"));
    ...
    UEFI_ASSERT_EXECUTION(NetworkTest::init(), UefiStatus::ABORTED);
    UEFI_LI(("Network Test initialized"));



    UEFI_ASSERT_EXECUTION(NetworkTestGUI::init(&params), UefiStatus::ABORTED);
    UEFI_LI(("Network Test GUI initialized"));



    UEFI_ASSERT_EXECUTION(NetworkTestGUI::exec(), UefiStatus::ABORTED); // Loop forever
