#ifndef OS_SHARED_COMMON_SRC_BITS64_CPU_CPUFEATURES_H
#define OS_SHARED_COMMON_SRC_BITS64_CPU_CPUFEATURES_H
...
#define AMOUNT_OF_WORDS_FOR_X86_FEATURES    16
...
#define CPUID_00000007_EDX                  5
#define CPUID_0000000D_1_EAX                6
#define CPUID_0000000F_0_EDX                7
#define CPUID_0000000F_1_EDX                8
#define CPUID_80000001_ECX                  9
#define CPUID_80000001_EDX                  10
#define CPUID_80000007_EBX                  11
#define CPUID_80000008_EBX                  12
...
enum class X86Feature: u16
{
    ...
    DSCPL               = WORD_BIT(CPUID_00000001_ECX, 4),                  // "ds_cpl" CPL-qualified (filtered) Debug Store
    VMX                 = WORD_BIT(CPUID_00000001_ECX, 5),                  // Hardware virtualization
    SMX                 = WORD_BIT(CPUID_00000001_ECX, 6),                  // Safer Mode eXtensions
    ...
    CX16                = WORD_BIT(CPUID_00000001_ECX, 13),                 // CMPXCHG16B instruction
    XTPR                = WORD_BIT(CPUID_00000001_ECX, 14),                 // Send Task Priority Messages
    PDCM                = WORD_BIT(CPUID_00000001_ECX, 15),                 // Perf/Debug Capabilities MSR
    ...
    XSAVE               = WORD_BIT(CPUID_00000001_ECX, 26),                 // XSAVE/XRSTOR/XSETBV/XGETBV instructions
    OSXSAVE             = WORD_BIT(CPUID_00000001_ECX, 27),                 // "" XSAVE instruction enabled in the OS
    AVX                 = WORD_BIT(CPUID_00000001_ECX, 28),                 // Advanced Vector Extensions
    F16C                = WORD_BIT(CPUID_00000001_ECX, 29),                 // 16-bit FP conversions
    RDRAND              = WORD_BIT(CPUID_00000001_ECX, 30),                 // RDRAND instruction
    ...
    //
    FSGSBASE            = WORD_BIT(CPUID_00000007_EBX, 0),                  // RDFSBASE, WRFSBASE, RDGSBASE, WRGSBASE instructions
    ...
    ERMS                = WORD_BIT(CPUID_00000007_EBX, 9),                  // Enhanced REP MOVSB/STOSB instructions
    ...
    RDSEED              = WORD_BIT(CPUID_00000007_EBX, 18),                 // RDSEED instruction
    ADX                 = WORD_BIT(CPUID_00000007_EBX, 19),                 // ADCX and ADOX instructions
    ...
    INTEL_PT            = WORD_BIT(CPUID_00000007_EBX, 25),                 // Intel Processor Trace
    ...
    UMIP                = WORD_BIT(CPUID_00000007_ECX, 2),                  // User Mode Instruction Protection
    ...
    AVX512_VBMI2        = WORD_BIT(CPUID_00000007_ECX, 6),                  // Additional AVX512 Vector Bit Manipulation Instructions
    GFNI                = WORD_BIT(CPUID_00000007_ECX, 8),                  // Galois Field New Instructions
    VAES                = WORD_BIT(CPUID_00000007_ECX, 9),                  // Vector AES
    VPCLMULQDQ          = WORD_BIT(CPUID_00000007_ECX, 10),                 // Carry-Less Multiplication Double Quadword
    AVX512_VNNI         = WORD_BIT(CPUID_00000007_ECX, 11),                 // Vector Neural Network Instructions
    AVX512_BITALG       = WORD_BIT(CPUID_00000007_ECX, 12),                 // Support for VPOPCNT[B,W] and VPSHUF-BITQMB instructions
    TME                 = WORD_BIT(CPUID_00000007_ECX, 13),                 // Intel Total Memory Encryption
    ...
    CLDEMOTE            = WORD_BIT(CPUID_00000007_ECX, 25),                 // CLDEMOTE instruction
    MOVDIRI             = WORD_BIT(CPUID_00000007_ECX, 27),                 // MOVDIRI instruction
    MOVDIR64B           = WORD_BIT(CPUID_00000007_ECX, 28),                 // MOVDIR64B instruction
    ...
    //
    AVX512_4VNNIW           = WORD_BIT(CPUID_00000007_EDX, 2),              // AVX-512 Neural Network Instructions
    AVX512_4FMAPS           = WORD_BIT(CPUID_00000007_EDX, 3),              // AVX-512 Multiply Accumulation Single precision
    PCONFIG                 = WORD_BIT(CPUID_00000007_EDX, 18),             // Intel PCONFIG
    SPEC_CTRL               = WORD_BIT(CPUID_00000007_EDX, 26),             // "" Speculation Control (IBRS + IBPB)
    INTEL_STIBP             = WORD_BIT(CPUID_00000007_EDX, 27),             // "" Single Thread Indirect Branch Predictors
    FLUSH_L1D               = WORD_BIT(CPUID_00000007_EDX, 28),             // Flush L1D cache
    ARCH_CAPABILITIES       = WORD_BIT(CPUID_00000007_EDX, 29),             // IA32_ARCH_CAPABILITIES MSR (Intel)
    SPEC_CTRL_SSBD          = WORD_BIT(CPUID_00000007_EDX, 31),             // "" Speculative Store Bypass Disable
    ...
    //
    XSAVEOPT            = WORD_BIT(CPUID_0000000D_1_EAX, 0),                // XSAVEOPT instruction
    XSAVEC              = WORD_BIT(CPUID_0000000D_1_EAX, 1),                // XSAVEC instruction
    XGETBV1             = WORD_BIT(CPUID_0000000D_1_EAX, 2),                // XGETBV with ECX = 1 instruction
    XSAVES              = WORD_BIT(CPUID_0000000D_1_EAX, 3),                // XSAVES/XRSTORS instructions
    ...
    //
    CQM_OCCUP_LLC       = WORD_BIT(CPUID_0000000F_1_EDX, 0),                // LLC occupancy monitoring
    ...
    PERFCTR_LLC         = WORD_BIT(CPUID_80000001_ECX, 28),                 // Last Level Cache performance counter extensions
    MWAITX              = WORD_BIT(CPUID_80000001_ECX, 29),                 // MWAIT extension (MONITORX/MWAITX instructions)
    ...
    LM                  = WORD_BIT(CPUID_80000001_EDX, 29),                 // Long Mode (x86-64, 64-bit support)
    ...
    XSAVEERPTR          = WORD_BIT(CPUID_80000008_EBX, 2),                  // Always save/restore FP error pointers
    AMD_IBPB            = WORD_BIT(CPUID_80000008_EBX, 12),                 // "" Indirect Branch Prediction Barrier
    AMD_IBRS            = WORD_BIT(CPUID_80000008_EBX, 14),                 // "" Indirect Branch Restricted Speculation
    AMD_STIBP           = WORD_BIT(CPUID_80000008_EBX, 15),                 // "" Single Thread Indirect Branch Predictors
    AMD_SSBD            = WORD_BIT(CPUID_80000008_EBX, 24),                 // "" Speculative Store Bypass Disable
    VIRT_SSBD           = WORD_BIT(CPUID_80000008_EBX, 25),                 // Virtualized Speculative Store Bypass Disable
    AMD_SSB_NO          = WORD_BIT(CPUID_80000008_EBX, 26),                 // "" Speculative Store Bypass is fixed in hardware
    ...
    VGIF                = WORD_BIT(CPUID_8000000A_EDX, 16),                 // Virtual GIF
...
#endif // OS_SHARED_COMMON_SRC_BITS64_CPU_CPUFEATURES_H
