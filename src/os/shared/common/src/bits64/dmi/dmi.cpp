...
NgosStatus DMI::init()
{
    ...
    // Validation
    {
...
#if NGOS_BUILD_COMMON_LOG_LEVEL == OPTION_LOG_LEVEL_INHERIT && NGOS_BUILD_LOG_LEVEL >= OPTION_LOG_LEVEL_VERY_VERY_VERBOSE || NGOS_BUILD_COMMON_LOG_LEVEL >= OPTION_LOG_LEVEL_VERY_VERY_VERBOSE
        {
            ...
            {
                COMMON_LVVV(("#%-3d: %-5s | %s | %s | %s | %s | %s", i, bytesToString(sMemoryDevices[i].size), stringToString(sMemoryDevices[i].deviceLocator), stringToString(sMemoryDevices[i].manufacturer), stringToString(sMemoryDevices[i].serialNumber), stringToString(sMemoryDevices[i].assetTag), stringToString(sMemoryDevices[i].partNumber)));
            }
...
NgosStatus DMI::saveDmiMemoryDeviceEntry(DmiEntryHeader *header)
{
    ...
    // Init memory device
    {
        ...
        {
            sMemoryDevices[memoryId].size = entryV27->extendedSize.size();
        }
        else
        {
