...
enum class PciDevice1543: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_3052 = 0x3052,
    DEVICE_4C22 = 0x4C22
...
inline const char8* enumToString(PciDevice1543 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1543 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1543 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
