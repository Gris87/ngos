...
enum class PciDevice193F: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0001 = 0x0001,
    DEVICE_0360 = 0x0360,
...
inline const char8* enumToString(PciDevice193F device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice193F device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice193F device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
