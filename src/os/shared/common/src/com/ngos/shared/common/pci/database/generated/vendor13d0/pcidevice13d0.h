...
enum class PciDevice13D0: u16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_2103 = 0x2103,
    DEVICE_2104 = 0x2104,
...
inline const char8* enumToString(PciDevice13D0 device13D0) // TEST: NO
{
    // COMMON_LT((" | device13D0 = %u", device13D0)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice13D0 device13D0) // TEST: NO
{
    // COMMON_LT((" | device13D0 = %u", device13D0)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device13D0, enumToString(device13D0));
...
inline const char8* enumToHumanString(PciDevice13D0 device13D0) // TEST: NO
{
    // COMMON_LT((" | device13D0 = %u", device13D0)); // Commented to avoid bad looking logs
