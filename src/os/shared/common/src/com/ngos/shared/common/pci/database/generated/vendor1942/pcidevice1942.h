...
enum class PciDevice1942: u16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_E511 = 0xE511,
    DEVICE_E521 = 0xE521
...
inline const char8* enumToString(PciDevice1942 device1942) // TEST: NO
{
    // COMMON_LT((" | device1942 = %u", device1942)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice1942 device1942) // TEST: NO
{
    // COMMON_LT((" | device1942 = %u", device1942)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device1942, enumToString(device1942));
...
inline const char8* enumToHumanString(PciDevice1942 device1942) // TEST: NO
{
    // COMMON_LT((" | device1942 = %u", device1942)); // Commented to avoid bad looking logs
