...
enum class PciDevice169C: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0044 = 0x0044
};
...
inline const char8* enumToString(PciDevice169C device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice169C device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice169C device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
