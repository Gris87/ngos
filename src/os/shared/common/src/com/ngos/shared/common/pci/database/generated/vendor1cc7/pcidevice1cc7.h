...
enum class PciDevice1CC7: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0200 = 0x0200,
    DEVICE_0250 = 0x0250
...
inline const char8* enumToString(PciDevice1CC7 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1CC7 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1CC7 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
