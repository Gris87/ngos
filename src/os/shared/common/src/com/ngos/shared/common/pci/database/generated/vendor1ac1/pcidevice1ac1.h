...
inline const char8* enumToString(PciDevice1AC1 device1AC1) // TEST: NO
{
    // COMMON_LT((" | device1AC1 = %u", device1AC1)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice1AC1 device1AC1) // TEST: NO
{
    // COMMON_LT((" | device1AC1 = %u", device1AC1)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device1AC1, enumToString(device1AC1));
...
inline const char8* enumToHumanString(PciDevice1AC1 device1AC1) // TEST: NO
{
    // COMMON_LT((" | device1AC1 = %u", device1AC1)); // Commented to avoid bad looking logs
