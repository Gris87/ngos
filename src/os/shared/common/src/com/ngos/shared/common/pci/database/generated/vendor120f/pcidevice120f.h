...
inline const char8* enumToString(PciDevice120F device120F) // TEST: NO
{
    // COMMON_LT((" | device120F = %u", device120F)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice120F device120F) // TEST: NO
{
    // COMMON_LT((" | device120F = %u", device120F)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device120F, enumToString(device120F));
...
inline const char8* enumToHumanString(PciDevice120F device120F) // TEST: NO
{
    // COMMON_LT((" | device120F = %u", device120F)); // Commented to avoid bad looking logs
