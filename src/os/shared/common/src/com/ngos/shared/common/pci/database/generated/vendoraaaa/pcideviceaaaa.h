...
enum class PciDeviceAAAA: u16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0001 = 0x0001,
    DEVICE_0002 = 0x0002
...
inline const char8* enumToString(PciDeviceAAAA deviceAAAA) // TEST: NO
{
    // COMMON_LT((" | deviceAAAA = %u", deviceAAAA)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDeviceAAAA deviceAAAA) // TEST: NO
{
    // COMMON_LT((" | deviceAAAA = %u", deviceAAAA)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)deviceAAAA, enumToString(deviceAAAA));
...
inline const char8* enumToHumanString(PciDeviceAAAA deviceAAAA) // TEST: NO
{
    // COMMON_LT((" | deviceAAAA = %u", deviceAAAA)); // Commented to avoid bad looking logs
