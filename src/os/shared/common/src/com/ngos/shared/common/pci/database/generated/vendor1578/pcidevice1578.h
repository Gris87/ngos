...
enum class PciDevice1578: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_4D34 = 0x4D34,
    DEVICE_5615 = 0x5615
...
inline const char8* enumToString(PciDevice1578 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1578 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1578 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
