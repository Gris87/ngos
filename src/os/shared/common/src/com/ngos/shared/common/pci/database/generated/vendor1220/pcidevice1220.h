...
enum class PciDevice1220: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_1220 = 0x1220
};
...
inline const char8* enumToString(PciDevice1220 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1220 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1220 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
