...
enum class PciDevice1036: bad_uint16 // Ignore CppEnumVerifier
{
    DEVICE_0000 = 0x0000
};
...
inline const char8* enumToString(PciDevice1036 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1036 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1036 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
