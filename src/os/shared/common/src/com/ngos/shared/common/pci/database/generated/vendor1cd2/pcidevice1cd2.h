...
enum class PciDevice1CD2: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0301 = 0x0301,
    DEVICE_0302 = 0x0302,
...
inline const char8* enumToString(PciDevice1CD2 device) // TEST: NO
{
    ...
    switch (device)
    {
        ...
        case PciDevice1CD2::DEVICE_0307: return "DEVICE_0307";
...
inline const char8* enumToFullString(PciDevice1CD2 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1CD2 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
    ...
    switch (device)
    {
        ...
        case PciDevice1CD2::DEVICE_0307: return "Simulyzer-RT CompactPCI Serial DIO-2 card [Xilinx Zynq UltraScale+]";
