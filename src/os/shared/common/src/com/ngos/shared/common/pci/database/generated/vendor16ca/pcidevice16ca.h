...
inline const char8* enumToString(PciDevice16CA device16CA) // TEST: NO
{
    // COMMON_LT((" | device16CA = %u", device16CA)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice16CA device16CA) // TEST: NO
{
    // COMMON_LT((" | device16CA = %u", device16CA)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device16CA, enumToString(device16CA));
...
inline const char8* enumToHumanString(PciDevice16CA device16CA) // TEST: NO
{
    // COMMON_LT((" | device16CA = %u", device16CA)); // Commented to avoid bad looking logs
