...
enum class PciDevice5046: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_1001 = 0x1001
};
...
inline const char8* enumToString(PciDevice5046 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice5046 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice5046 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
