...
enum class PciDevice0EAC: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0008 = 0x0008
};
...
inline const char8* enumToString(PciDevice0EAC device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice0EAC device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice0EAC device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
