...
enum class PciDevice118D: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0001 = 0x0001,
    DEVICE_0012 = 0x0012,
...
inline const char8* enumToString(PciDevice118D device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice118D device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice118D device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
