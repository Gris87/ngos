...
inline const char8* enumToString(PciDevice15DC device15DC) // TEST: NO
{
    // COMMON_LT((" | device15DC = %u", device15DC)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice15DC device15DC) // TEST: NO
{
    // COMMON_LT((" | device15DC = %u", device15DC)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device15DC, enumToString(device15DC));
...
inline const char8* enumToHumanString(PciDevice15DC device15DC) // TEST: NO
{
    // COMMON_LT((" | device15DC = %u", device15DC)); // Commented to avoid bad looking logs
