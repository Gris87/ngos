...
enum class PciDevice111F: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_4A47 = 0x4A47,
    DEVICE_5243 = 0x5243
...
inline const char8* enumToString(PciDevice111F device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice111F device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice111F device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
