...
enum class PciDevice12C5: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_007E = 0x007E,
    DEVICE_007F = 0x007F,
...
inline const char8* enumToString(PciDevice12C5 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice12C5 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice12C5 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
