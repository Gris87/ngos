...
inline const char8* enumToString(PciDevice12D2 device12D2) // TEST: NO
{
    // COMMON_LT((" | device12D2 = %u", device12D2)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice12D2 device12D2) // TEST: NO
{
    // COMMON_LT((" | device12D2 = %u", device12D2)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device12D2, enumToString(device12D2));
...
inline const char8* enumToHumanString(PciDevice12D2 device12D2) // TEST: NO
{
    // COMMON_LT((" | device12D2 = %u", device12D2)); // Commented to avoid bad looking logs
