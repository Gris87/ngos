...
enum class PciDevice1074: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_4E78 = 0x4E78
};
...
inline const char8* enumToString(PciDevice1074 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1074 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1074 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
