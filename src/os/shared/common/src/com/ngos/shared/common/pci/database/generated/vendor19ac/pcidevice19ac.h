...
inline const char8* enumToString(PciDevice19AC device19AC) // TEST: NO
{
    // COMMON_LT((" | device19AC = %u", device19AC)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice19AC device19AC) // TEST: NO
{
    // COMMON_LT((" | device19AC = %u", device19AC)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device19AC, enumToString(device19AC));
...
inline const char8* enumToHumanString(PciDevice19AC device19AC) // TEST: NO
{
    // COMMON_LT((" | device19AC = %u", device19AC)); // Commented to avoid bad looking logs
