...
enum class PciDevice166D: u16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0001 = 0x0001,
    DEVICE_0002 = 0x0002,
...
inline const char8* enumToString(PciDevice166D device166D) // TEST: NO
{
    // COMMON_LT((" | device166D = %u", device166D)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice166D device166D) // TEST: NO
{
    // COMMON_LT((" | device166D = %u", device166D)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device166D, enumToString(device166D));
...
inline const char8* enumToHumanString(PciDevice166D device166D) // TEST: NO
{
    // COMMON_LT((" | device166D = %u", device166D)); // Commented to avoid bad looking logs
