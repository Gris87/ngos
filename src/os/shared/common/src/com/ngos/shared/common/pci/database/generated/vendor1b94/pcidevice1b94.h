...
enum class PciDevice1B94: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_E400 = 0xE400
};
...
inline const char8* enumToString(PciDevice1B94 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1B94 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1B94 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
