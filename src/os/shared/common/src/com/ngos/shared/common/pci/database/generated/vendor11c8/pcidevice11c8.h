...
enum class PciDevice11C8: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0658 = 0x0658,
    DEVICE_D665 = 0xD665,
...
inline const char8* enumToString(PciDevice11C8 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice11C8 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice11C8 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
