...
enum class PciDevice106C: u16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_8139 = 0x8139,
    DEVICE_8801 = 0x8801,
...
inline const char8* enumToString(PciDevice106C device106C) // TEST: NO
{
    // COMMON_LT((" | device106C = %u", device106C)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice106C device106C) // TEST: NO
{
    // COMMON_LT((" | device106C = %u", device106C)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device106C, enumToString(device106C));
...
inline const char8* enumToHumanString(PciDevice106C device106C) // TEST: NO
{
    // COMMON_LT((" | device106C = %u", device106C)); // Commented to avoid bad looking logs
