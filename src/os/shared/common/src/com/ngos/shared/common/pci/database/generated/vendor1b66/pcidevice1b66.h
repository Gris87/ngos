...
enum class PciDevice1B66: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0007 = 0x0007
};
...
inline const char8* enumToString(PciDevice1B66 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1B66 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1B66 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
