...
inline const char8* enumToString(PciDevice170B device170B) // TEST: NO
{
    // COMMON_LT((" | device170B = %u", device170B)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice170B device170B) // TEST: NO
{
    // COMMON_LT((" | device170B = %u", device170B)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device170B, enumToString(device170B));
...
inline const char8* enumToHumanString(PciDevice170B device170B) // TEST: NO
{
    // COMMON_LT((" | device170B = %u", device170B)); // Commented to avoid bad looking logs
