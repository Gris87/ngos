...
enum class PciDevice14A4: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_22F1 = 0x22F1,
    DEVICE_4318 = 0x4318
...
inline const char8* enumToString(PciDevice14A4 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice14A4 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice14A4 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
