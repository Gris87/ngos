...
enum class PciDevice1267: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_5352 = 0x5352,
    DEVICE_5A4B = 0x5A4B
...
inline const char8* enumToString(PciDevice1267 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1267 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1267 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
