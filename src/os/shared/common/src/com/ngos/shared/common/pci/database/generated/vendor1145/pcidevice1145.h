...
enum class PciDevice1145: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_8007 = 0x8007,
    DEVICE_F007 = 0xF007,
...
inline const char8* enumToString(PciDevice1145 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1145 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1145 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
