...
enum class PciDevice1C7F: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_5100 = 0x5100
};
...
inline const char8* enumToString(PciDevice1C7F device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1C7F device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1C7F device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
