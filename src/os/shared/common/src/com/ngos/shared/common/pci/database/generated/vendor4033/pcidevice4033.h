...
enum class PciDevice4033: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_1360 = 0x1360
};
...
inline const char8* enumToString(PciDevice4033 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice4033 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice4033 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
