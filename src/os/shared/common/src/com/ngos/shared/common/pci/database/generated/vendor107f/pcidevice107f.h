...
enum class PciDevice107F: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0802 = 0x0802
};
...
inline const char8* enumToString(PciDevice107F device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice107F device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice107F device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
