...
enum class PciDevice11B9: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_C0ED = 0xC0ED
};
...
inline const char8* enumToString(PciDevice11B9 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice11B9 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice11B9 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
