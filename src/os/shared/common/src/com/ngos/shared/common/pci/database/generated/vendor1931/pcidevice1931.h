...
enum class PciDevice1931: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_000C = 0x000C
};
...
inline const char8* enumToString(PciDevice1931 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1931 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1931 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
