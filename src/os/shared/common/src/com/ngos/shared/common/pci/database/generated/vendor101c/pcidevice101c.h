...
enum class PciDevice101C: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0193 = 0x0193,
    DEVICE_0196 = 0x0196,
...
inline const char8* enumToString(PciDevice101C device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice101C device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice101C device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
