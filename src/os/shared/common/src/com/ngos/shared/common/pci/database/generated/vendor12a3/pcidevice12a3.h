...
enum class PciDevice12A3: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_8105 = 0x8105
};
...
inline const char8* enumToString(PciDevice12A3 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice12A3 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice12A3 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
