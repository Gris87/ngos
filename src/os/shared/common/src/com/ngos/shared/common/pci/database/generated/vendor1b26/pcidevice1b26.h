...
enum class PciDevice1B26: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_C132 = 0xC132,
    DEVICE_C1C0 = 0xC1C0,
...
inline const char8* enumToString(PciDevice1B26 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1B26 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1B26 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
