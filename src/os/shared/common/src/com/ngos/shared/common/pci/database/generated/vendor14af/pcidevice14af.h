...
enum class PciDevice14AF: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_7102 = 0x7102
};
...
inline const char8* enumToString(PciDevice14AF device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice14AF device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice14AF device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
