...
enum class PciDevice10FC: u16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0003 = 0x0003,
    DEVICE_0005 = 0x0005
...
inline const char8* enumToString(PciDevice10FC device10FC) // TEST: NO
{
    // COMMON_LT((" | device10FC = %u", device10FC)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice10FC device10FC) // TEST: NO
{
    // COMMON_LT((" | device10FC = %u", device10FC)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device10FC, enumToString(device10FC));
...
inline const char8* enumToHumanString(PciDevice10FC device10FC) // TEST: NO
{
    // COMMON_LT((" | device10FC = %u", device10FC)); // Commented to avoid bad looking logs
