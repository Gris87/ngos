...
enum class PciDevice1CE4: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0001 = 0x0001,
    DEVICE_0002 = 0x0002,
...
inline const char8* enumToString(PciDevice1CE4 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1CE4 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1CE4 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
