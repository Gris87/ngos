...
NgosStatus Console::init(BootParams *params, RgbaPixel *doubleBuffer, u64 doubleBufferSize)
{
    ...
    sLastLineBuffer     = (RgbaPixel *)((address_t)sDoubleBuffer + doubleBufferSize - sLastLineBufferSize);
    ...
    if (ch >= 0x20 && ch < 0x7F)
    {
        ...
        for (i64 i = 0; i < glyphData->bitmapHeight; ++i)
        {
            for (i64 j = 0; j < glyphData->bitmapWidth; ++j)
            {
                ...
                COMMON_TEST_ASSERT((address_t)pixel >= (address_t)sLastLineBuffer
                                    &&
                                    (address_t)pixel <= (address_t)sLastLineBuffer + sLastLineBufferSize - sizeof(RgbaPixel), NgosStatus::ASSERTION);
