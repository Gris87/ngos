...
inline const char8* enumToString(PciDevice1B3A device1B3A) // TEST: NO
{
    // COMMON_LT((" | device1B3A = %u", device1B3A)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice1B3A device1B3A) // TEST: NO
{
    // COMMON_LT((" | device1B3A = %u", device1B3A)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device1B3A, enumToString(device1B3A));
...
inline const char8* enumToHumanString(PciDevice1B3A device1B3A) // TEST: NO
{
    // COMMON_LT((" | device1B3A = %u", device1B3A)); // Commented to avoid bad looking logs
