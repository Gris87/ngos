...
inline const char8* enumToString(PciDevice13C2 device13C2) // TEST: NO
{
    // COMMON_LT((" | device13C2 = %u", device13C2)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice13C2 device13C2) // TEST: NO
{
    // COMMON_LT((" | device13C2 = %u", device13C2)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device13C2, enumToString(device13C2));
...
inline const char8* enumToHumanString(PciDevice13C2 device13C2) // TEST: NO
{
    // COMMON_LT((" | device13C2 = %u", device13C2)); // Commented to avoid bad looking logs
