...
enum class PciDevice1497: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_1497 = 0x1497
};
...
inline const char8* enumToString(PciDevice1497 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1497 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1497 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
