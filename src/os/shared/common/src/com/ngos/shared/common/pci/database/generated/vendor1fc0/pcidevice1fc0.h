...
enum class PciDevice1FC0: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0300 = 0x0300,
    DEVICE_0301 = 0x0301
...
inline const char8* enumToString(PciDevice1FC0 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1FC0 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1FC0 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
