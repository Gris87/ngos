...
enum class PciDevice16C6: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_8695 = 0x8695,
    DEVICE_8842 = 0x8842
...
inline const char8* enumToString(PciDevice16C6 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice16C6 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice16C6 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
