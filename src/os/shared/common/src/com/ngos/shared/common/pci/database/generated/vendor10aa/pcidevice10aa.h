...
inline const char8* enumToString(PciDevice10AA device10AA) // TEST: NO
{
    // COMMON_LT((" | device10AA = %u", device10AA)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice10AA device10AA) // TEST: NO
{
    // COMMON_LT((" | device10AA = %u", device10AA)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device10AA, enumToString(device10AA));
...
inline const char8* enumToHumanString(PciDevice10AA device10AA) // TEST: NO
{
    // COMMON_LT((" | device10AA = %u", device10AA)); // Commented to avoid bad looking logs
