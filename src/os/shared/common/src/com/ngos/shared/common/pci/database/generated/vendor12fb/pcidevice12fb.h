...
enum class PciDevice12FB: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0001 = 0x0001,
    DEVICE_00F5 = 0x00F5,
...
inline const char8* enumToString(PciDevice12FB device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice12FB device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice12FB device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
