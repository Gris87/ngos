...
enum class PciDevice1D37: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0013 = 0x0013,
    DEVICE_0014 = 0x0014,
...
inline const char8* enumToString(PciDevice1D37 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1D37 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1D37 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
