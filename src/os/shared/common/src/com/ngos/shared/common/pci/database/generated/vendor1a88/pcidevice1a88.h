...
enum class PciDevice1A88: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_4D45 = 0x4D45
};
...
inline const char8* enumToString(PciDevice1A88 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1A88 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1A88 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
