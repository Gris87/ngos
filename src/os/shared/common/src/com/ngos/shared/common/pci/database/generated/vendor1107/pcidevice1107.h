...
enum class PciDevice1107: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0576 = 0x0576
};
...
inline const char8* enumToString(PciDevice1107 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1107 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1107 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
