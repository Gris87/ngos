...
enum class PciInterface0C00: bad_uint8 // Ignore CppEnumVerifier
{
    INTERFACE_00 = 0x00,
    INTERFACE_10 = 0x10
};
...
inline const char8* enumToString(PciInterface0C00 interface) // TEST: NO
{
...
inline const char8* enumToFullString(PciInterface0C00 interface) // TEST: NO
{
    // COMMON_LT((" | interface = %u", interface)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciInterface0C00 interface) // TEST: NO
{
    // COMMON_LT((" | interface = %u", interface)); // Commented to avoid bad looking logs
