...
inline const char8* enumToString(PciDevice199F device199F) // TEST: NO
{
    // COMMON_LT((" | device199F = %u", device199F)); // Commented to avoid bad looking logs
...
inline const char8* enumToFullString(PciDevice199F device199F) // TEST: NO
{
    // COMMON_LT((" | device199F = %u", device199F)); // Commented to avoid bad looking logs
    ...
    sprintf(res, "0x%04X (%s)", (u16)device199F, enumToString(device199F));
...
inline const char8* enumToHumanString(PciDevice199F device199F) // TEST: NO
{
    // COMMON_LT((" | device199F = %u", device199F)); // Commented to avoid bad looking logs
