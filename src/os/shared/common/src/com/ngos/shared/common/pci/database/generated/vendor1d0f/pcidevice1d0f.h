...
enum class PciDevice1D0F: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_CD01 = 0xCD01,
    DEVICE_EC20 = 0xEC20,
...
inline const char8* enumToString(PciDevice1D0F device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1D0F device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1D0F device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
