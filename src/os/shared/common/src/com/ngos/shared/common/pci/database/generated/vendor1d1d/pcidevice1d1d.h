...
enum class PciDevice1D1D: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_1F1F = 0x1F1F,
    DEVICE_2807 = 0x2807
...
inline const char8* enumToString(PciDevice1D1D device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1D1D device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1D1D device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
