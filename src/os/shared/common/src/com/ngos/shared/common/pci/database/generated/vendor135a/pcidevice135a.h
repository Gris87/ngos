...
enum class PciDevice135A: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0A61 = 0x0A61
};
...
inline const char8* enumToString(PciDevice135A device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice135A device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice135A device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
