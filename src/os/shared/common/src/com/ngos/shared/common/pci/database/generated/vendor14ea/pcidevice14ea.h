...
enum class PciDevice14EA: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_AB06 = 0xAB06,
    DEVICE_AB07 = 0xAB07,
...
inline const char8* enumToString(PciDevice14EA device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice14EA device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice14EA device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
