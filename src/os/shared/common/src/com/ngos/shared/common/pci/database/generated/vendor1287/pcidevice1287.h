...
enum class PciDevice1287: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_001E = 0x001E,
    DEVICE_001F = 0x001F
...
inline const char8* enumToString(PciDevice1287 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1287 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1287 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
