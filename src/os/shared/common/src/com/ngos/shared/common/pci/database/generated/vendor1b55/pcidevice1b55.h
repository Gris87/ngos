...
enum class PciDevice1B55: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_18F6 = 0x18F6,
    DEVICE_18F7 = 0x18F7,
...
inline const char8* enumToString(PciDevice1B55 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1B55 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1B55 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
