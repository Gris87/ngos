...
enum class PciDevice126C: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_1211 = 0x1211,
    DEVICE_126C = 0x126C
...
inline const char8* enumToString(PciDevice126C device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice126C device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice126C device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
