...
enum class PciDevice6688: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_1200 = 0x1200,
    DEVICE_1400 = 0x1400,
...
inline const char8* enumToString(PciDevice6688 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice6688 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice6688 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
