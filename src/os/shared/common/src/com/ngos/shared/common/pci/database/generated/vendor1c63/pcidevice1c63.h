...
enum class PciDevice1C63: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_0008 = 0x0008
};
...
inline const char8* enumToString(PciDevice1C63 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1C63 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1C63 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
