...
enum class PciDevice1204: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_1965 = 0x1965
};
...
inline const char8* enumToString(PciDevice1204 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice1204 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice1204 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
