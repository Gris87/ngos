...
NgosStatus Png::loadImage(u8 *data, u64 size, bool withNinePatch, Image **image)
{
    ...
    *image = nullptr;
    ...
    PngChunk   *chunk  = (PngChunk *)((address_t)pngHeader + sizeof(PngHeader) + sizeof(PngImageHeader) + 4); // 4 == size of CRC
    ...
    while (status == NgosStatus::OK && size >= sizeof(PngChunk) + 4) // 4 == size of CRC
    {
        ...
        chunk =  (PngChunk *)((address_t)chunk + totalChunkLength);
...
NgosStatus Png::decodeChunk(PngDecoder *decoder, PngChunk *chunk, u32 chunkLength)
{
    ...
    u32 crc32Checksum = ntohl(*(u32 *)((address_t)&chunk->data + chunkLength));
