...
enum class PciDevice9412: bad_uint16 // Ignore CppEnumVerifier
{
    NONE        = 0,
    DEVICE_6565 = 0x6565
};
...
inline const char8* enumToString(PciDevice9412 device) // TEST: NO
{
...
inline const char8* enumToFullString(PciDevice9412 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
...
inline const char8* enumToHumanString(PciDevice9412 device) // TEST: NO
{
    // COMMON_LT((" | device = %u", device)); // Commented to avoid bad looking logs
